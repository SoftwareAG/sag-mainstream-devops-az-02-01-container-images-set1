...
buildah push "${JOB_CONTAINER_POSTGRES_VARIANT_MAIN_TAG}"
...
if [[ "${BUILD_SOURCEBRANCHNAME}" == "main" ]]; then
  ...
  buildah tag "${JOB_CONTAINER_POSTGRES_VARIANT_MAIN_TAG}" "${JOB_CONTAINER_POSTGRES_VARIANT_BASE_TAG}"

  logI "Pushing tag ${JOB_CONTAINER_POSTGRES_VARIANT_BASE_TAG}"
  buildah push "${JOB_CONTAINER_POSTGRES_VARIANT_BASE_TAG}"
fi
